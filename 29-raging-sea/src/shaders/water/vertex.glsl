...
void main() {
  ...
  for (float i = 1.0; i < 3.0; i += 0.7) {
    smallWaves += abs(
      cnoise(
        vec3(
          modelPosition.x * uSmallWavesFrequency.x * i,
          uTime * uSmallWavesSpeed,
          modelPosition.z * uSmallWavesFrequency.y * i
        )
      )
    ) * -1.0 * uSmallWavesAmplitude / i;
  }

  float elevation = bigWaves + smallWaves;

  modelPosition.y += elevation;
  vElevation = (elevation + uColorOffset) * uColorMultiplier;
